...
public class DeviceSsidDailyUsagesChainMemberMgrImpl implements ConsolidateJobsChainMemberMgr {
	private static final Logger log = LoggerFactory.getLogger(DeviceSsidDailyUsagesChainMemberMgrImpl.class);
		...
		try{
			if (consolidateJobCommonParamsDto != null){
				if (consolidateJobCommonParamsDto.getJobName() != null && 
						consolidateJobCommonParamsDto.getJobName().equals(ConsolidateJob.JOB_NAME_DEVICE_SSID_DAILY_USAGES_CONSOLIDATION)){
					boolean areParamsValided = areParamsValided(consolidateJobCommonParamsDto);
					if (!areParamsValided){
						log.warn("DeviceSsidDailyUsagesChainMemberMgrImpl.responseJobOrPass() - areParamsValided:" + areParamsValided);
						return;
					}
					...
					sb.append("DeviceSsidDailyUsagesChainMemberMgrImpl - no of records:" + noOfRecordsDone + ", ");
			...
			log.error("DeviceSsidDailyUsagesChainMemberMgrImpl.responseJobOrPass() - Exception: ", e);
		}
	}
