...
public class DeviceDailyUsagesChainMemberMgrImpl implements ConsolidateJobsChainMemberMgr {
	private static final Logger log = LoggerFactory.getLogger(DeviceDailyUsagesChainMemberMgrImpl.class);
		...
		try{
			if (consolidateJobCommonParamsDto != null){
				if (consolidateJobCommonParamsDto.getJobName() != null && 
						consolidateJobCommonParamsDto.getJobName().equals(ConsolidateJob.JOB_NAME_DEVICE_DAILY_USAGES_CONSOLIDATION)){
					boolean areParamsValided = areParamsValided(consolidateJobCommonParamsDto);
					if (!areParamsValided){
						log.warn("DeviceDailyUsagesChainMemberMgrImpl.responseJobOrPass() - areParamsValided:" + areParamsValided);
						return;
					}
			...
			log.error("DeviceDailyUsagesChainMemberMgrImpl.responseJobOrPass() - Exception: ", e);
		}
	}
